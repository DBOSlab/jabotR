...
#' may supply a specific path to previously downloaded files using \code{path},
#' or let the function handle the download automatically. Filters can be applied
#' by taxon name, herbarium code, state, and year. The \code{reorder} argument
#' allows you to customize the structure of the returned data. Use
#' \code{verbose = TRUE} to see status updates. Use \code{save = TRUE} to save
#' results to disk as a CSV file in the specified directory.
#'
#' @note
...
#' and criteria from the chosen JABOT herbaria. If \code{save = TRUE}, the function
