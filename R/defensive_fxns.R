...
.arg_check_path <- function(path, dwca_folders, dwca_filenames) {
  ...
  if (!dir.exists(path)) {
    ...
    if (!any(grepl("^dwca", dwca_folders))) {
      stop(paste0("There is no JABOT-downloaded dwca folder within the directory '", path, "'."),
           call. = FALSE)
    } else {
...
.arg_check_herbarium <- function(x, verbose = verbose) {
  ...
  # Get valid herbarium acronyms from JABOT metadata
  ...
  if (length(invalid) > 0) {
    stop(
      sprintf(
        "The following herbarium acronym(s) are not recognized by JABOT: %s
        \nUse `jabot_summary()` to view available collections.",
